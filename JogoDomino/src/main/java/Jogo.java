...
public class Jogo {
    ...
    Lista PecasDisponiveis = gerarPecas();
    public void IniciarJogo(){
        Menu menu = new Menu();
        Outputs outputs = new Outputs();
        Lista pecasHumano = DistribuirPecas();
        Lista pecasComputador = DistribuirPecas();
    ...
    public Lista DistribuirPecas(){
        int controle = 0;
        Random rand = new Random();
        Lista lista = new Lista();
        while(controle != 8){
            Peca peca = PecasDisponiveis.IdentificarPecaPorIndice(rand.nextInt(27));
            lista.inserir(peca);
            PecasDisponiveis.removerPeca(peca);
            controle++;
